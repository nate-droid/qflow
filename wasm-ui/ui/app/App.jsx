...
// --- NEW --- Component to visualize the phase of a complex amplitude
const PhaseDisk = ({ re, im, size = 24 }) => {
    // Only render the disk if the probability is non-negligible
    const probability = re * re + im * im;
    if (probability < 1e-6) {
        // Render a placeholder to maintain layout alignment
        return <div style={{ width: size, height: size }} className="flex-shrink-0" />;
    }

    // atan2 gives the angle in radians. Convert to degrees for CSS transform.
...
// --- UPDATED --- SimulationOutput component now includes PhaseDisk
            ...
            try {
                ...
                const mockWasm = {
                    run_simulation: (circuitJson) => {
                        console.log("Mock WASM received:", circuitJson);
                        const circuit = JSON.parse(circuitJson);

                        // Special case for a 2-qubit Bell state: H on q0, then CNOT q0->q1
                        if (circuit.numQubits === 2 && circuit.moments.length === 2) {
                            const isBellState = circuit.moments[0]?.[0]?.type === 'H' && circuit.moments[0]?.[0]?.qubit === 0 && circuit.moments[1]?.[0]?.type === 'CNOT';
                            if (isBellState) {
                        ...
                        // Default case: return |00...0> state
