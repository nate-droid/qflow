  ...
  elements: [
    // Nodes
    {
      data: {
        ...
        status: "Succeeded",
        type: "classical",
        params: { generator: "make_moons", samples: 100 },
      },
    },
    {
      data: {
  ...
  const handleFetchWorkflow = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/workflows/${workflowNameInput}`,
      );
      if (!response.ok) {
    ...
    return (
      <div className="absolute inset-0 bg-slate-900/80 flex items-center justify-center z-30">
        <div className="w-full max-w-md p-4 text-center">
          <h2 className="mt-2 text-2xl font-bold text-white mb-2">
            Fetch a Workflow
          </h2>
          <p className="text-slate-400 mb-6">
            Enter the name and namespace of a workflow to visualize.
          </p>
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            <input
              ...
              className="bg-slate-800 border border-slate-600 rounded-md py-2 px-3 text-white w-full"
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 font-semibold disabled:opacity-50"
          >
