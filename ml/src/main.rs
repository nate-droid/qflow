...
//     use super::Complex;
//
//     /// A placeholder for a quantum gate. In a real scenario, this would
//     /// be a more complex enum or struct representing different quantum operations
//     /// like Hadamard, CNOT, Pauli-X, etc., along with their target qubits.
//     #[derive(Debug)]
//     pub struct Gate {
//         pub name: String,
//         pub target_qubit: usize,
//         // Rotation gates often have an angle parameter.
//         pub parameter: Option<f64>,
//     }
//
//     /// The Simulator trait defines the essential capabilities of a quantum simulator.
//     pub trait Simulator {
//         /// Resets the simulator to the all-zeros state: |0...0⟩.
//         fn reset(&mut self);
//         /// Applies a single quantum gate, modifying the internal statevector.
//         fn apply_gate(&mut self, gate: &Gate);
//         /// Returns a reference to the current quantum statevector.
//         fn get_statevector(&self) -> &Vec<Complex<f64>>;
//     }
//
//     /// A simple, illustrative implementation of a quantum simulator.
//     pub struct BasicSimulator {
//         statevector: Vec<Complex<f64>>,
//         num_qubits: usize,
//     }
//
//     impl BasicSimulator {
//         pub fn new(num_qubits: usize) -> Self {
//             let dim = 1 << num_qubits; // Statevector dimension is 2^n
//             let mut statevector = vec![Complex::new(0.0, 0.0); dim];
//             // Initialize to |0...0⟩ state, which is (1, 0, 0, ...)
...
//         fn reset(&mut self) {
//             // Set all amplitudes to zero except for the |0...0⟩ state.
//             for val in self.statevector.iter_mut() {
//                 *val = Complex::new(0.0, 0.0);
//             }
//             self.statevector[0] = Complex::new(1.0, 0.0);
//         }
//
//         fn apply_gate(&mut self, gate: &Gate) {
//             // This is a highly simplified mock of gate application.
//             // A real simulator would apply a unitary matrix corresponding to the gate.
//             // Here, we'll just deterministically alter the state for demonstration.
//             // For example, a rotation gate might mix amplitudes based on its parameter.
//             if let Some(param) = gate.parameter {
//                 // A mock "rotation" that mixes the first two states
//                 let c = Complex::new(param.cos(), 0.0);
//                 let s = Complex::new(param.sin(), 0.0);
//                 let v0 = self.statevector[0];
//                 let v1 = self.statevector[1];
//                 self.statevector[0] = c * v0 - s * v1;
//                 self.statevector[1] = s * v0 + c * v1;
//             }
//             // Normalize the statevector to maintain a total probability of 1.
//             let norm_sq: f64 = self.statevector.iter().map(|c| c.norm_sqr()).sum();
//             let norm = norm_sq.sqrt();
//             if norm > 0.0 {
//                 for c in self.statevector.iter_mut() {
//                     *c /= norm;
//                 }
//             }
//         }
